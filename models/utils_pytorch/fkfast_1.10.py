...
class lct_fk_fast(nn.Module):
    ...
    def __init__(self, spatial=256, crop=512, \
        ...
        super(lct_fk_fast, self).__init__()
        ...
        self.register_buffer("datazero_Dx2Tx2Hx2W", torch.zeros((dnum, 2 * self.crop, 2 * self.spatial_grid, 2 * self.spatial_grid), dtype=torch.float32), persistent=False)
        self.register_buffer("newcoord_dx2Mx2Nx2Nx3_todev", self.newcoord_1x2Mx2Nx2Nx3.repeat(dnum, 1, 1, 1, 1), persistent=False)
        self.register_buffer("gridz_2Mx2Nx2N_todev", self.gridz_2Mx2Nx2N.repeat(dnum, 1, 1, 1, 1), persistent=False)
        self.register_buffer("gridznew_todev", self.gridznew.repeat(dnum, 1, 1, 1, 1), persistent=False)
    ...
    def forward(self, feture_bxdxtxhxw, tbes, tens):
        ...
        datafre_temp = torch.fft.fftn(torch.complex(datapad_BDx2Tx2Hx2W,datazero_BDx2Tx2Hx2W), dim = (-4,-3,-2,-1))
...
if __name__ == '__main__':
    ...
    lctlayer = lct_fk_fast(spatial=sptial_grid, crop=crop, bin_len=bin_len).cuda()
