...
def normalize(data_bxcxdxhxw):   #  min max scaling
...
if __name__ == '__main__':
    ...
    if syn:
        ...
        mat = False
        if video:
            videofile = '/data/liyue/FK/LFE_render/NLOS_EF_allviews_scale_0.75processed/0/ff1fb5a9dd255b059036436a751dc6c2/shine_0.0000-rot_8.9286_-62.6221_5.4496-shift_0.1621_0.3305_-0.1085/video-confocal-gray-full.mp4'
        ...
        from scipy.io import loadmat  # teaser0  statue0  resolution0.mat dragon0.mat  bike0.mat
        data = loadmat(file_name='/data/liyue/FK/fk_lfe/datasets/bike0.mat')
    ...
    gau = 0.05 + 0.03 * torch.randn_like(rect_data_bxdxhxwxt) + rect_data_bxdxhxwxt
    rect_data_bxdxhxwxt = 0.03 * torch.randn_like(rect_data_bxdxhxwxt) * gau + gau
    # transform = get_transform(1,[0.05,0.5])
    # rect_data_bxdxhxwxt = rect_data_bxdxhxwxt.permute([0,1,4,2,3])
    # rect_data_bxdxhxwxt = transform(rect_data_bxdxhxwxt)
    # rect_data_bxdxhxwxt = rect_data_bxdxhxwxt.permute([0,1,3,4,2])
    # rect_data_bxdxhxwxt = rect_data_bxdxhxwxt.data.cpu().numpy()
    # rect_data_bxdxhxwxt = torch.from_numpy(rect_data_bxdxhxwxt).to(dev)
    print(rect_data_bxdxhxwxt.shape)
    rect_data_bxdxhxwxt = normalize(rect_data_bxdxhxwxt.permute([0,1,4,2,3])).permute([0,1,3,4,2])
    #####################################################################
    ...
    re2 = re * 2
    re2[re2 > 1] = 1
    cv2.imwrite("re.png", re*255)
    cv2.imwrite("re2.png", re2*255)
    cv2.imwrite("re256.png", re*255)
    # cv2.imshow("gt", imgt)
    ...
    dep_mat = front_index.astype(np.float32)/np.max(front_index)
    import matplotlib.pyplot as plt
    import scipy.io as sio
    plt.imshow(dep_mat)
    str_name = 'lct_bike0'
    plt.savefig(str_name + '.png')
    sio.savemat(str_name+ '.mat',{'dep_pre':dep_mat})

    front_index_show = front_index.astype(np.uint8)
    cv2.imwrite("dep.png", front_index_show)       ## 这里可能有问题 正负？
    #cv2.waitKey()
    ...
    ### 后续应该是 对intensity的后处理   去除周围一些没用的index 然后重新索引
    for i in range(-5, 6):
        idx = front_index + i
        idx[idx < 0] = 0
        idx[idx >= d] = d - 1 
    ...
    for i, frame in enumerate(layers):
        ...
        cv2.imwrite("layer1.png", 5 * frame)
        cv2.imwrite("layer2.png", (frame / np.max(frame))*255)
        # cv2.waitKey(0)
        
    for i, frame in enumerate(volumn_ZxYxX):
        #pass
        #print(i)
        cv2.imwrite("re1.png", 2 * cv2.resize(frame, (256, 256)))
        cv2.imwrite("re2.png", (frame / np.max(frame))*255)
        # cv2.waitKey(0)
